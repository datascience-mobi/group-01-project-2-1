---
title: "Final Presentation, Lapatinib"
Subtitle: "Project02 - Group01"
...
The Kruskal-Wallis test is for independent samples. It tests whether the central trends of several independent samples differ. Since the p-value of the Kruskal-Wallis test is significant, our biomarkers do not show the same central tendencies. It can be conclude that the genes are not closely related.
...
<div style="text-align: justify"> 
### 3.2.2 Visualization of the genome-wide paired t-test in a volcano plot {#css_class3.2.2}

If the p-value of a gene is less than 0.05, it turns red. Thus, all significant genes are stained red.</div>
...
volcano<-data.frame(genes=Lgenes,Fold=Lapatinib_fc, pvalue=padj)
names(volcano)[names(volcano) == "colMeans.LapUn...LapTreat."] <- "log2FoldChange"
abslog2FoldChange<-abs(volcano$log2FoldChange)
```
```{r, echo=TRUE, eval=FALSE}
with(volcano, plot(log2FoldChange, -log10(pvalue), pch=20, main="Volcano plot", xlim=c(-2.5,2)))
```
```{r, fig.height=12, fig.width=12, message=TRUE, paged.print=TRUE}
with(volcano, plot(log2FoldChange, -log10(pvalue), pch=20, main="Volcano plot", xlim=c(-2.5,2)))
# Add colored points: red if padj<0.05, blue if log2FC>1, green if both)
with(subset(volcano, padj<.05 ), points(log2FoldChange, -log10(pvalue), pch=20, col="red"))
with(subset(volcano, (abslog2FoldChange)>1), points(log2FoldChange, -log10(pvalue), pch=20, col="blue"))
with(subset(volcano, padj<.05 & (abslog2FoldChange)>1), points(log2FoldChange, -log10(pvalue), pch=20, col="green"))
# Label points with the textxy function from the calibrate plot
with(subset(volcano, padj<.05 & (abslog2FoldChange)>1), textxy(log2FoldChange, -log10(pvalue), labs = genes, cex=.8))
```
