...
palette(magma(15))
par(mar=c(5, 4, 5, 9))
boxplot(Treated1, border=adjustedMeda$drug, 
        xlab= "Samples" ,
...
As you can see the scaling is necessary to compensate the batch effect. Scaling is then applyed on Untreated aswell before furhter analysis.

### Densityplot 
As a other method to get in touch with the data, we checked if there is a differenc between the treated and untreated genexpression. 
...
As assumed, the expressionpaterns don`t differentiate much.

###k-means clustering
To look for clusters in the raw data we performed a k-menas clustering and searched for potentially clusters using the most variable (>75% Var), thus informative genes. Summary and dimmension of the the found genes:
```{r, echo=FALSE}
