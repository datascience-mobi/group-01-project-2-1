...
plot(dat1.train$NegLogI50Lap, l1.train$fitted.values, pch = 20, col = "blue", xlab = "Real values", 
...
plot(dat2.train$NegLogI50Lap, l2.train$fitted.values, pch = 20, col = "blue", xlab = "Real values", 
...
plot(dat3.train$NegLogI50Lap, l3.train$fitted.values, pch = 20, col = "blue", xlab = "Real values", 
...
Rsrd =  c(summary(l1.train)$r.squared, summary(l2.train)$r.squared, summary(l3.train)$r.squared)
F_Stat = c(summary(l1.train)$fstatistic[1],summary(l2.train)$fstatistic[1],summary(l3.train)$fstatistic[1])
Sum_RMSE = c(rmse1.test,rmse2.test,rmse3.test)
Rsrd1 = as.data.frame(t(Rsrd))
table = rbind(Rsrd1,F_Stat,Sum_RMSE)
colnames(table) = c("Foldchange","Doublingtime","Multiple regression")
...
```{r plot erlotinib, fig.height=8, fig.width=8, echo = FALSE}
...
```{r two plots, fig.height=10, fig.width=16, echo = FALSE}
...
```{r main question, lung genes, echo = FALSE}
...
```{r plot both, fig.height=6, fig.width=14, echo = FALSE}
# Plot Lapatinib

e = ggplot(dif.NegLogGI50.lung,aes(x = celllines, y = erlotinib)) + geom_bar(stat = "identity", fill = "skyblue") + geom_text(aes(label=round(lapatinib, 2)), vjust = -0.5, color = "black", size = 3) + coord_flip() + labs(title = "Mean graph plot of NLGI50 values for Erlotinib, only lung genes")
