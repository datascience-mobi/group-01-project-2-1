...
[4. Main questions](#anchor4)  
  [4.1 Question 1: Does the cell doubling time correlate with reduced drug sensitivity?](#css_id4.1)  
    [4.1.1 Linear regression ~ Inoculation_Density](#css_class4.1.1)    
    [4.1.2 Linear regression ~ Doubling-time](#css_class4.1.2)  
    [4.1.3 Multiple regression](#css_class4.1.3)  
    [4.1.4 Table of conclusion](#css_class4.1.4)    
...
In order to colour our clusters according to sample annotations, we created a colour vector by dissecting the metadata matrix by our PCA data samples. Additionally we were fixing some labelling problems. 
...
Subsequently we created the colour vectors for colouring according to drug and tissue type. 
...
To determine clusters of samples based on their similarity, we plotted PC1 and PC2 as well as a PC2 and PC3 while colouring by tissue type and drug respectively.
...
As seen in the scree plot, the first four PCs have capture most of the information since the curve bends at an "elbow" at PC4 - this is our cutting off point.
...
In order to colour our clusters according to sample annotations, we created a colour vector by dissecting the metadata matrix as well as the cell-line-annotation matrix  by our PCA data samples. 
...
Subsequently we created the colour vectors for colouring according to drug and microsatellite instability status.
...
To determine clusters of samples based on their similarity, we plotted PC1 and PC2 aswell as a PC2 and PC3 while colouring by microsatellite instability status and drug respectively.

```{r, fig.height = 4.5, fig.width = 7.2, warning = FALSE}
...
F_Stat = c(summary(l22.train)$fstatistic[1],summary(l2.train)$fstatistic[1],summary(l3.train)$fstatistic[1])
...
The heat map shows, that the top expression genes are regulated similarly in breast and in cns cells after treatment with Lapatinib. 
