...
[1. Broad analysis](#anchor1)

    [Boxplots](#css_class1.1)
    
    [Densityplot](#css_class1.2)
    
    [k-means clustering](#css_class1.3)
    
    [PCA](#css_class1.4)


[2. Specific analysis: lapatinib](#anchor2)
...
[3. Main questions](#anchor3)

  [Question 1: Does the cell doubling time correlate with reduced drug sensitivity?](#css_id3.1)
  ...
  [Question 2: Does Lapatinib have a similar effect on lung cancer as Erlotinib?](#css_id3.2)
  ...
  [Question 3: Comparing lapatinib treated breast and cns celllines](#css_id3.3)
...
##Analysis of biomarker and t-test between treated and untreated cells
...
<div style="text-align: justify">  
We found these genes as biomarkers in a paper and presented them in our first presentation. Unfortunately, we can not use them as a biomarker.</div>
...
```
<div style="text-align: justify"> 
GDF3 Growth Differentiation Factor 3 belongs to the transforming growth factor beta (TGF-β) superfamily and has some intrinsic activity.
...
<div style="text-align: justify"> 
###Boxplot of our biomarkers
</div>
```{r, fig.height=8, fig.width=18, message=TRUE, paged.print=TRUE, echo=FALSE}
...
df_breast<-data.frame(expression, cells)

```

<div style="text-align: justify"> 
###ggBoxplot of our biomarkers' basalexpression in breast cells with Kruskal-Wallis test
The null hypothesis is: there is no difference between the groups.
The Kruskal-Wallis test is for independent samples. The test tests whether the central trends of several independent samples differ. </div>
...
###Heatmap of the biomarker in breast cells
...
Our biomarkers are significant.</div>
```{r, table1, echo=FALSE}
library(matrixTests)
library(data.table)
...
<div style="text-align: justify"> 
###Visualization of the genome-wide paired t-test in a vulcano plot
</div>
...
As you can see from the plot, there is a positive correlation between inoculation density and doubling time. This means that the higher the inoculation density, the higher the doubling time, which means that the tumour grows slowly because the cells need more time to divide.



# 3. Main questions {#anchor}
...
<div style="text-align: justify"> 
p = 0.53 means that the result is not significant. Thus, the two drugs did not differ significantly from each other.</div>
```{r}
